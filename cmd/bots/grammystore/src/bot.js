...
export function fulfillmentForSale(sale, starsRate = 20) {
  ...
  if (product.kind === KINDS.stars) {
    const titleAmount = Number(String(sale.title).match(/^(\d+)\s+[^ ]+ Stars$/)?.[1] ?? 0);
...
const ID_HELP_TEXT = "ℹ️ <b>Как получить свой Telesrv ID</b>\n\n1. Перейдите в бот <b>@getmyid</b> (именно в <b>Telesrv</b>, не в Telegram!)\n2. Нажмите «Старт»\n3. Скопируйте свой ID\n4. Пришлите его сюда сообщением";
...
function mainKeyboard(admin) {
  const keyboard = new InlineKeyboard()
    ...
    .text("🆔 Как узнать свой Telesrv ID", "idhelp");
...
function shopKeyboard() {
  return new InlineKeyboard()
    ...
    .text("⭐ Telesrv Stars", "cat:stars").row()
...
function adminKeyboard() {
  return new InlineKeyboard()
    ...
    .text("💱 Курс Telesrv Stars", "adm:rate").row()
...
export function createBot({ config, db, gramsrv }) {
  ...
  async function fulfill(product, recipientID, buyer, chatID, chargeID, extra = "") {
    ...
    if (product.kind === KINDS.premium) {
      if (!positiveInteger(recipientID)) throw new Error("Некорректный Telesrv ID получателя");
      ...
      if (!positiveInteger(recipientID) || !username) throw new Error("Некорректный Telesrv ID или @username");
    ...
    if (number) {
      ...
      const result = product.kind === KINDS.premium ? `Premium на ${product.months} мес.` : `${product.starsAmount} Telesrv Stars`;
  ...
  bot.on("message:successful_payment", async (ctx) => {
    ...
    try {
      if (payment.invoice_payload.startsWith("custom|")) {
        ...
        if (product.kind === KINDS.stars) {
          if (parsed.starsAmount <= 0) throw new Error("В счёте не зафиксировано количество Telesrv Stars");
          product = { ...product, starsAmount: parsed.starsAmount, title: `${parsed.starsAmount} Telesrv Stars` };
        }
  ...
  bot.on("callback_query:data", async (ctx) => {
    ...
    if (data === "shop") { db.clearPending(ctx.from.id); return editOrReply(ctx, "🛍 <b>Telesrv Shop</b>\n\nВыбери категорию:", shopKeyboard()); }
    ...
    if (data.startsWith("adm:")) {
      ...
      const prompts = {
        stars: ["give_stars", "⭐ <b>Выдача Telesrv Stars</b>\n\nПришли: <code>telesrv_id количество</code>"],
        premium: ["give_premium", "💎 <b>Выдача Premium</b>\n\nПришли: <code>telesrv_id месяцы</code>"],
        username: ["give_username", "🎭 <b>Выдача коллекционного @username</b>\n\nПришли: <code>telesrv_id username ставка_TON</code>"],
        ...
        rate: ["rate_set", `💱 <b>Курс Telesrv Stars</b>\n\nТекущий: 1⭐ = <b>${db.starsRate()}</b> Telesrv Stars.\n\nПришли новое число.`],
      };
    ...
    if (data.startsWith("cat:")) {
      ...
      if (kind === KINDS.stars) {
        ...
        return editOrReply(ctx, `⭐ <b>Покупка Telesrv Stars</b>\n\nАктуальный курс: <b>1⭐ Telegram = ${rate} Telesrv Stars</b>\n\nСколько <b>Telegram Stars</b> вы хотите потратить? Пришли число сообщением.\n\nНапример: <code>100</code> → получите ${100 * rate} Telesrv Stars.`, backMenuKeyboard());
      }
      const titles = { premium: "💎 <b>Telesrv Premium</b>\n\nВыбери срок:", number: "📞 <b>Анонимные номера +888</b>\n\nВыбери формат:", username: "🎭 <b>Коллекционные @username</b>\n\nВыбери ставку:" };
  ...
  bot.on("message:text", async (ctx) => {
    ...
    try {
      ...
      if (pending.kind === "stars_amount") {
        ...
        db.setPending(ctx.from.id, "target_id", { productCode: code });
        return keep(`Отлично — счёт будет на <b>${amount} ⭐ Telegram</b>, зачислим <b>${amount * db.starsRate()} Telesrv Stars</b>.\n\n🆔 Теперь пришли <b>Telesrv ID</b> получателя.\n\n${ID_HELP_TEXT}`, targetKeyboard(code, ctx.from.id, db));
      }
      ...
      if (pending.kind === "spin_id") {
        ...
        db.clearPending(ctx.from.id);
        return keep(`🎰 <b>Крутим...</b>\n\n🎉 Поздравляем! Выпало <b>${award.prize} ⭐ Telesrv Stars</b>\n\nНачислено на Telesrv ID <code>${serverUserID}</code>.`, mainKeyboard(isOwner(config, ctx.from.id)));
      }
      if (pending.kind === "promo_claim") {
        const serverUserID = positiveInteger(input);
        if (!serverUserID) return keep(`Неверный Telesrv ID.\n\n${ID_HELP_TEXT}`);
        ...
        db.clearPending(ctx.from.id);
        return keep(`✅ Промокод активирован! Начислено <b>${promo.stars_amount} Telesrv Stars</b> на ID <code>${serverUserID}</code>`, mainKeyboard(isOwner(config, ctx.from.id)));
      }
      if (pending.kind === "gw_claim") {
        ...
        db.clearPending(ctx.from.id);
        return keep(`✅ Начислено <b>${item.stars_amount} Telesrv Stars</b> на ID <code>${serverUserID}</code>`, mainKeyboard(isOwner(config, ctx.from.id)));
      }
      ...
      if (pending.kind === "give_stars") {
        ...
        if (!id || !amount) return keep("Формат: <code>telesrv_id количество</code>");
        await gramsrv.grantStars(id, amount, "Telegram bot administrator grant", operationID); db.clearPending(ctx.from.id);
        return keep(`✅ Выдано ${amount} Telesrv Stars → ID <code>${id}</code>`);
      }
      if (pending.kind === "give_premium") {
        ...
        if (!id || !months) return keep("Формат: <code>telesrv_id месяцы</code>");
      ...
      if (pending.kind === "give_username") {
        ...
        if (!id || !username || !bid) return keep("Формат: <code>telesrv_id username ставка_TON</code>");
      ...
      if (pending.kind === "rate_set") {
        ...
        return keep(`✅ Курс обновлён: 1⭐ Telegram = <b>${rate}</b> Telesrv Stars`);
      }
      ...
      if (pending.kind === "gw_max") {
        ...
        return keep("Шаг 3/3. Пришли <b>количество Telesrv Stars</b> за одну активацию.");
      }
      ...
      if (pending.kind === "promo_new") {
        ...
        return keep(`✅ Промокод <code>${escapeHTML(promo.code)}</code> создан. Награда: ${promo.stars_amount} Telesrv Stars, макс. активаций: ${promo.max_acts}.`);
      }
    } catch (error) {
